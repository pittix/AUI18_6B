...
%DIF DEL paper.tex             Tue Jul 17 17:27:51 2018
%DIF ADD paper_corrected.tex   Fri Jul 20 12:43:58 2018
...
\numberofauthors{4}
\author{%
  \DIFdelbegin %DIFDELCMD < \alignauthor{Leave Authors Anonymous\\
%DIFDELCMD <     \affaddr{for Submission}\\
%DIFDELCMD <     \affaddr{City, Country}\\
%DIFDELCMD <     \email{e-mail address}}%%%
\DIFdelend \DIFaddbegin \alignauthor{Philipp Ubrich\\
    \affaddr{T6A - AUI - TU-Dresden}\\
    \affaddr{Dresden, Germany}\\
    \email{s9706290@mail.zih.tu-dresden.de}}\DIFaddend \\
  \DIFdelbegin %DIFDELCMD < \alignauthor{Leave Authors Anonymous\\
%DIFDELCMD <     \affaddr{for Submission}\\
%DIFDELCMD <     \affaddr{City, Country}\\
%DIFDELCMD <     \email{e-mail address}}%%%
\DIFdelend \DIFaddbegin \alignauthor{Fabian Mischke\\
    \affaddr{T6A - AUI - TU-Dresden}\\
    \affaddr{Dresden, Germany}\\
    \email{s0189852@mail.zih.tu-dresden.de}}\DIFaddend \\
  \DIFdelbegin %DIFDELCMD < \alignauthor{Leave Authors Anonymous\\
%DIFDELCMD <     \affaddr{for Submission}\\
%DIFDELCMD <     \affaddr{City, Country}\\
%DIFDELCMD <     \email{e-mail address}}%%%
\DIFdelend \DIFaddbegin \alignauthor{Andrea Pittaro\\
    \affaddr{T6A - AUI - TU-Dresden}\\
    \affaddr{Dresden, Germany}\\
    \email{andrea.pittaro@studenti.unipd.it}}\DIFaddend \\
  \DIFdelbegin %DIFDELCMD < \alignauthor{Leave Authors Anonymous\\
%DIFDELCMD <     \affaddr{for Submission}\\
%DIFDELCMD <     \affaddr{City, Country}\\
%DIFDELCMD <     \email{e-mail address}}%%%
\DIFdelend \DIFaddbegin \alignauthor{Philip Vogel\\
    \affaddr{T6A - AUI - TU-Dresden}\\
    \affaddr{Dresden, Germany}\\
    \email{philip.vogel@mailbox.tu-dresden.de}}\DIFaddend \\
}

\maketitle

\begin{abstract}
  Smart Textiles and body worn controllers (Wearables) have been analyzed, mostly individually, using different technologies and studying different aspects. Although there were some tests concerning everyday life, these controllers are not used very often for several reasons. This paper aims to show some of \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{these reasons }\DIFaddend and analyze, instead, the opportunities that  may lead to better development and improve the feeling that a user has when interacting \DIFdelbegin \DIFdel{in an uncommon way }\DIFdelend with a device \DIFaddbegin \DIFadd{in a way that's not common along the rest of the population}\DIFaddend . One of the main threats to the adoption of Wearables that this paper will discuss will be the reasons for social unacceptability. Other topics will be the methods to review social acceptability and possible solutions to the problems concerning the usage of Wearables in public, taking into account the technology-related tradeoffs.
  To illustrate this issue, different Wearables are going to be presented - specifically concentrating on the social aspects of the devices. Finally \DIFdelbegin \DIFdel{, }\DIFdelend this paper discusses further development of social acceptability of Wearables in the future.
\end{abstract}
...
Touch gestures are a form of interaction that is used \DIFdelbegin \DIFdel{by almost everyone daily }\DIFdelend \DIFaddbegin \DIFadd{a lot }\DIFaddend in our society.\DIFaddbegin \footnote{\DIFadd{To date, just counting only Android and iOS devices, more than 3 billion devices are active monthly, https://apple.co/2GD96NO and https://twitter.com/Google/status/864890655906
070529}} \DIFaddend Whether Smartphone, Tablet, or Smartwatch - many mobile devices are nowadays controlled by this input technique to master different tasks. High demands are directly related to the fast and intuitive use of these devices. The combination of different electronic devices can help achieve these requirements to a higher extent. For example, smart watches are already used to control mobile phones or various home electronics (e.g. controlling lamps). The development of innovative input devices, which can be integrated into clothing or even applied to the user's skin, offers the potential to communicate with electronic devices in completely new ways and make interaction even more intuitive. In this paper, we especially focus on touch input devices that are attached to the user's skin or are integrated into clothing. This can be used to control different electronic devices.
Because these technologies create new and unusual forms of interaction, we will also look at third-party perceptions of a user’s interactions with a wearable \DIFdelbegin \DIFdel{E-textile }\DIFdelend \DIFaddbegin \DIFadd{E-Textile }\DIFaddend interface. Social conventions play an important role in the development of these new forms of interaction. The willingness to make a gesture depends largely on how appropriate these interactions look and feel in public. \cite{touch-wrist} The usability of these technologies must also be examined: to support the user as efficiently, effectively and satisfactorily as possible, the designers must consider the interaction with the system in addition to the placement of these \DIFdelbegin \DIFdel{body-worn controllers}\DIFdelend \DIFaddbegin \DIFadd{wearables}\DIFaddend .
...
``Belt'' \cite{belt} is a body-worn controller specifically built to interact with Augmented Reality applications. As the name suggests, the device is worn around the hip like a regular belt\DIFaddbegin \DIFadd{, }\DIFaddend which is touch-sensitive and can differentiate between simple gestures and locate the position of the touch-input \cite[p. 2136]{belt}.%([8] Page 2136).
...
Another solution is presented by iSkin \cite{iSkin}, where a different material is used to allow input to be sensed.  \DIFdelbegin \DIFdel{The polydimethylsiloxane }\DIFdelend \DIFaddbegin \DIFadd{Polydimethylsiloxane }\DIFaddend (PDMS), the silicon-based material used, is “fully transparent, elastic, and a highly biocompatible material. [...] Therefore it is widely used on or in the human body, for example in body implants.” \cite[p. 2994]{iSkin} PDMS is non-conductive, but adding carbon produces cPDMS, which can create a conductive surface. This allows the creation of a gray-colored sensitive surface that can have a specific shape or design. \DIFdelbegin \DIFdel{. }\DIFdelend This feature can immediately let the user know what to press for an action, but only registers that the \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{area }\DIFaddend has been touched, without any information about the gesture made \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend that area. The reason for this limitation is the electrical principle used: the RC charge-discharge circuit allows time evaluation only for the whole sensitive surface \footnote{An RC electronic circuit has a typical charge-discharge time constant $\tau$ which is given by the product $R \cdot C$. when pressure is applied, capacitance changes as the distance between the two surfaces \DIFdelbegin \DIFdel{it’s }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend reduced. When a different charging time is sensed, the pressure is identified.}.
...
\cite{vogel4} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend %DIF >  \cite{vogel4} #TODO
...
use would  be deemed ineffective. These efforts are directed towards introducing more natural and expressive user interfaces for existing hardware as well as methods to influence gesture recognition by improving the quality of input. \cite{motion-ui, vogel7} %[10, 11]
...
\DIFdelbegin \subsection{\DIFdel{Methods to review social acceptability}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Methods to review social acceptability}}
\DIFaddend %
...
Since social acceptability is a major issue when it comes to the adaptation of new technology there is also the need to measure it appropriately. Different \DIFdelbegin \DIFdel{researches have different }\DIFdelend \DIFaddbegin \DIFadd{research have various }\DIFaddend approaches to review the social acceptability of their respective research. \cite{touch-wrist}

\DIFdelbegin \section{\DIFdel{Testing - Demonstrating Technology}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Testing with dummy (without working technology)}
...
When reviewing the social acceptability of a product and on a larger scale even the product itself\DIFaddbegin \DIFadd{, }\DIFaddend at last it comes down to testing with the device itself. Tests with a dummy are inherently easy to set up and do not require costly prototypes. They can also yield interesting, insightful results, especially when it comes to reviewing the gestures and overall appearance. On the other hand there are some aspects you can only truly examine while testing with working technology. These aspects include for example the actual amount of distraction the user experiences while using the device. A survey participant might experience some minor distraction while practicing a gesture with a dummy and simultaneously talking to a person. When he now repeats the same gesture with working \DIFdelbegin \DIFdel{a working }\DIFdelend Augmented Reality Wearable he might encounter a stronger level of distraction due to the Augmented Reality Overlays\DIFaddbegin \DIFadd{, }\DIFaddend which are shown to him.
...
\DIFdelbegin \subsubsection{\DIFdel{Demo video showing technology}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Demo video showing technology}}
\DIFaddend % Don’t mind Me, The Social Comfort, Usable Gestures
...
\label{tab:interview}
\begin{tabular}{ll}
\textbf{Multiple choice interview}                   & \textbf{Conversation-style interview}                            \\ \hline
Belt (likert)                       & Belt                                                    \\
...
This quote underlines the current challenge Wearables have to face to succeed in the long term. Since as mentioned by Dunne, Lucy E., et al. \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{social-comfort} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend the basic needs are figured out for Wearables they now have to please higher needs - such as social acceptability. To illuminate the current state some results of current \DIFdelbegin \DIFdel{researches }\DIFdelend \DIFaddbegin \DIFadd{research }\DIFaddend are presented here.
...
Result “Don’t Mind me Touching my Wrist” \cite{touch-wrist}: The "Jogwheel"  \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{touch-wrist} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend mentioned above was used in an online survey to evaluate the social acceptance of an \DIFdelbegin \DIFdel{E-textile }\DIFdelend \DIFaddbegin \DIFadd{E-Textile }\DIFaddend interface by showing a demo video to the participants. The results showed gender and cultural differences in this field. There were large gender-specific differences, especially in body placement. For example, while the placement of the interface on a man's chest looked very accessible and natural to the participants, the same part of a woman's body was perceived as “disgusting”. \cite{touch-wrist} The participants from the USA found the interaction with the interface at the waist by a man more embarrassing than by a woman. The answers of the South Koreans were exactly reversed here, which indicates \DIFdelbegin \DIFdel{to }\DIFdelend cultural differences. \cite{touch-wrist}

Overall, the results of this study show a gender effect in reference to the placement and interaction of an \DIFdelbegin \DIFdel{E-textile}\DIFdelend \DIFaddbegin \DIFadd{E-Textile}\DIFaddend . Especially the waist (for men) and the chest (for women) revealed to be socially sensitive areas. In contrast, the body positions of forearm and wrist were rated very positively, which could also be due to existing technologies (wristwatches, e-straps).
...
Result “Social Comfort of Wearable Technology” \cite{social-comfort} : When it comes to the placement of Wearables there is a strong aversion to inappropriate body locations. While preferred positions are mainly chosen because \DIFaddbegin \DIFadd{of }\DIFaddend easy accessibility, positions are mainly eluded to avoid embarrassment. Also the visual properties of a Wearable are a key factor for acceptance.
...
The single most prominent factor mentioned in the respective research projects, besides the type of gesture, was the body region where the Wearable was placed. The figure \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:body} }\DIFaddend shows the preferred regions in the individual studies. When comparing the \DIFdelbegin \DIFdel{researches }\DIFdelend \DIFaddbegin \DIFadd{research }\DIFaddend there are two main observations to be made. First there is a strict aversion against socially suggestive regions for Wearables. These regions include genitalia, the bottom and, especially when asking female subjects, the upper torso area, including the sternum and collarbone.
